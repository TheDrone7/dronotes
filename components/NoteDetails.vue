<template>
  <box bg="gray.900" px='12' py="4" border-top="solid 1px" border-color="gray.700" display="flex" align-items="center" justify-content="space-between">
  ...
  methods: {
    ...
    async deleteNote(nid) {
      ...
      if (deleted.success) {
        ...
        this.$store.commit('setNotes', notes.filter(n => n.id !== nid));
