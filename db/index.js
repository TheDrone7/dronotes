...
export const createUser = async (email, username, password) => {
  ...
  if (checkUser && !checkUser.error)
    return { error: 'This username is already in use. Please use a new username or login if it is your own username.' };
  await userDoc
    .set({
      email,
      username,
...
export const fetchUserByName = async username => {
  ...
  if (userDoc.size)
    return {
      ...
      timeCreated: new Date(userDoc.docs[0].createTime.seconds * 1000),
      ...userDoc.docs[0].data()
    };
...
export const fetchUser = async id => {
  ...
  if (userDoc.exists)
    return { id: userDoc.id, timeCreated: new Date(userDoc.createTime.seconds * 1000), ...userDoc.data() };
  else return { error: 'User does not exist.' };
};
...
export const fetchNote = async nid => {
  ...
  if (noteDoc.exists)
    return { id: noteDoc.id, timeCreated: new Date(noteDoc.createTime.seconds * 1000), ...noteDoc.data() };
